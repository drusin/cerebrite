...
- **Only the secret goes in the store.** A non-secret **connection record** sits in `.git/cerebrite/connection.json`, inside the repository's git dir. It is never committed, moves with the folder, and is never picked up by a fresh clone. It holds the connection ID, credential kind, HTTPS username, provider, token expiry, author identity and which credential store is in use. `origin` stays in `.git/config`. `settings.json` stays app-level and gains only an index of connection IDs and their stores.
